...
function initFlowEditor(item){
  setOnly=false;scheduleOnly='';$('#editor').classList.remove('set-editor');
...
function saveFlow(next,item,old){
  ...
  rule.task={...structuredClone(item),done:false,completedOn:''};delete rule.task.followUpOf;delete rule.task.repeatId;delete rule.task.repeatDate;
}
...
function renderDailyProgress(){
 ...
 const first=nextHomeTask();document.querySelectorAll('[data-home-task]').forEach(el=>{const i=find(el.dataset.homeTask);el.classList.toggle('up-next',i.id===first?.id);el.querySelector('small').textContent=(i.id===first?.id?'今やる · ':'')+(i.priority===2?'最重要 · ':i.priority===1?'重要 · ':'')+taskDate(i);});
}
...
document.addEventListener('click',async e=>{
 ...
 if(b.dataset.editSchedule){const r=data.schedules.find(s=>s.id===b.dataset.editSchedule);$('#flow-manager').close();openEditor('task');scheduleOnly=r.id;$('#editor-title').textContent='今後の繰り返しを編集';$('#item-title').value=r.task.title;$('#item-date').value=r.nextDate;$('#item-body').value=r.task.body;$('#item-action').value=r.task.actionTarget||'';$('#item-priority').value=String(r.task.priority||0);fillResources(r.task.resources);$('#resource-fields').open=!!hasResources(r.task.resources);$('#item-repeat').value=r.active?r.frequency:'none';formStart='schedule-edit';}
